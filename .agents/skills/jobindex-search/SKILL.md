...
- Hjemmearbejde (remote) is UI-only — there is no `--remote` flag. Remote passes must omit the city and use keywords (`remote`, `hjemmearbejde`); classify from `detail`. Do not invent an API parameter for it.
