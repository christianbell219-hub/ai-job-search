...
WRITABLE_STATUSES = frozenset(CANONICAL_STATUSES - {"in_progress"})
...
def classify_row(row: dict[str, str], today: date, silent_days: int = SILENT_DAYS) -> str:
    ...
    if status == "applied" and applied is not None:
        if today - applied >= timedelta(days=silent_days):
            ...
            if not re.search(r"\b(replied|reply|interview|offer|follow-?up sent)\b", notes):
                return "silent"
    return "waiting"
            ...
            ranked.append(
                {
                    ...
                    "rank_gaps": entry.get("rank_gaps") or [],
        ...
        applications.append(
            {
                **row,
                ...
                "commands": {
                    ...
                    "followup": f"/interview {row['company']} --followup",
