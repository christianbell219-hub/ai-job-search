...
- **Remote / workplace:** if the portal has a remote or workplace-type parameter, add `--remote` (or the portal's equivalent) and document the values in SKILL.md so `/scrape` can pass them. If it does **not** (UI-only filter, or no filter at all), write that plainly: **"unsupported — post-filter from detail"** — do not invent query parameters the API ignores (Jobindex hjemmearbejde is the precedent). `/scrape` will then classify `work_mode` from location/description after `detail`.
...
- **`SKILL.md` frontmatter:** `name`, `version: 1.0.0`, `enabled: true` (or `enabled: false` to skip the portal in `/scrape` without deleting the skill), a `description` written for skill triggering - it must name the portal, the market, and include trigger phrases in English **and** the market's language; `context: fork`; `allowed-tools: Bash(bun run skills/<name>/cli/src/cli.ts *)`.
