...
- **`/rank`** bridges `/scrape` and `/apply`: it batch-scores all newly scraped postings against the fit framework (parallel agents fetch each posting and score the five evaluation dimensions) and returns a ranked shortlist with honest per-job strengths and gaps. On top of the fit-quality score it computes a **Practical Fit Ranking Layer** — salary, remote fit, visa/work-authorization fit, and skill match — so you also see "can I actually take it?" beside "is it a good role?"; sort by either with `--by quality` (default) or `--by practical`. Deal-breakers veto (location relocation, visa no-sponsorship), deadlines get urgency flags, dead postings get marked expired. Pick a number and it hands off to the full `/apply` workflow.
