...
- **`/refine`** is the feedback loop that makes the search smarter as real results come in. It learns from *outcomes*, not just applications: it reads your resolved applications from the tracker (interviews and offers = strong positive, rejections and silence = negative) via a deterministic engine (`tools/preference_model.py`), and proposes sharper `/scrape` queries pointed at the role types, companies, and sectors that actually convert — including adjacent directions to explore. It **waits for outcomes**: until a few applications resolve (record them with `/outcome`), it stays quiet rather than refining on pending applications. `/scrape` reads the model to bias future runs, and query changes are always proposed for your approval, never applied silently.
...
│   │   ├── refine.md                  # /refine learn from your actions, sharpen search queries
...
│   ├── preference_model.py            # Revealed-preference engine powering /refine
...
├── job_scraper/                       # Scraper state (seen jobs, preferences, results)
