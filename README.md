...
- **`/status`** is a read-only pipeline view in Claude (tracker buckets, scrape backlog, deadline urgency). Complements the local board below.
- **Local board (optional):** `python3 tools/dashboard.py` opens http://127.0.0.1:8765 — a localhost-only Kanban over the same tracker + scrape backlog. It surfaces a **Needs action** strip (silent follow-ups, drafted deadlines, top ranked `/apply` copies), a **Claude rail** of copy-to-Claude commands with Gmail last-sync, a **paste inbox** for files in `documents/postings/`, can **Mark submitted** (`drafted` → `applied`), change tracker status, and toggle portal `enabled` flags; drafting still happens in Claude. Complements `/html-report` (static snapshot) and `/notion-sync` (remote glance).
...
│   │   ├── status.md                  # /status read-only pipeline view
...
├── tools/
│   ├── dashboard.py                   # Localhost pipeline board (python3 tools/dashboard.py)
│   ├── job_pipeline.py                # Tracker / seen_jobs helpers for the board and /status
...
├── dashboard/                         # Static UI for the local board
