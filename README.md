...
- **`/refine`** is the feedback loop that makes the search smarter the more you use it. It reads what you have been drawn to (applied-to, evaluated, and highly ranked jobs) versus what you pass on (skipped, low-ranked), distills an accumulating "taste" model with a deterministic engine (`tools/preference_model.py`), and proposes sharper `/scrape` queries — including adjacent role directions to explore. It learns from your *actions* (no resolved outcomes needed) and sharpens with every scrape, rank, and application; `/scrape` reads the model to bias future runs. Query changes are always proposed for your approval, never applied silently.
